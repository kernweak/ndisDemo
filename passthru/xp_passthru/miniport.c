...
//		DbgPrint(("Packet!=NULL"));
		...
		{ 
			DbgPrint("MPSent:NdisAllocateMemory Failed\n"); 
			return(NDIS_STATUS_NOT_ACCEPTED); 
		} 
		if(pPacketContent == NULL) 
		{ 
			DbgPrint("MPsent:pPacketContent==NULL\n"); 
			return(NDIS_STATUS_NOT_ACCEPTED); 
		} 
		NdisZeroMemory(pPacketContent, BUFFER_SIZE);

		CopyPacket2Buffer(Packet,pPacketContent,&OffsetSize);	//把分片的数据拷贝回数据包
	    if(OffsetSize!=0)
		{
				nMatch=UTIL_MatchRule(pPacketContent, DT_OUT);
		}
	
		//		DBGPRINT(("In MPSent And Free Memory\n")); 
...
//---------------------------------------------------------add by xiaoqiang Mar 12 2006
	    ...
	    if(Packet!=NULL)
		{
		    ...
		    if(OffsetSize!=0)
			{
					nMatch=UTIL_MatchRule(pPacketContent, DT_OUT);//解析数据得到ip端口，从out匹配规则
			}
...

{
	...
	DbgPrint("==>MPQueryPNPCapabilities(...)\n");
...
--*/
{
...
#endif // NDIS51
	{
		...
		PNDIS_PACKET	MyPacket;
		PRECV_RSVD		RecvRsvd;
//----------------------------------------------------------add by xiaoqiang Mar 16	
		RecvRsvd = (PRECV_RSVD)(Packet->MiniportReserved); 
		MyPacket = RecvRsvd->OriginalPkt;        //为了很好地释放内存,这个保留区就派上用场了
