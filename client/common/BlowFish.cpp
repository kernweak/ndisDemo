...
int CBlowFish::Pad(char* in, int iLength)
{
	int iRes = iLength%m_nBlockSize;
	if(iRes != 0)
	{
		//Is the caller's responsability to ensure that in buffer is large enough
		int iPadded = m_nBlockSize - iRes;
		char* pin = in+iLength;
		switch(m_nPadding)
		{
			case ZEROES:
...
void CBlowFish::Char2Hex(unsigned char ch, char* szHex)
{
...
bool CBlowFish::Hex2Char(char const* szHex, unsigned char& rch)
{
	if(*szHex >= '0' && *szHex <= '9')
		rch = *szHex - '0';
...
void CBlowFish::Binary2Hex(unsigned char const* pucBinStr, int iBinSize, char* pszHexStr)
{
	int i;
	char szHex[3];
...
bool CBlowFish::Hex2Binary(char const* pszHexStr, unsigned char* pucBinStr, int iBinSize)
{
	int i;
	unsigned char ch;
...
inline BYTE CBlowFish::Uint2Byte(unsigned int ui)
{
...
{
	return ((m_auiS[0][Uint2Byte(ui>>24)] + m_auiS[1][Uint2Byte(ui>>16)]) ^ m_auiS[2][Uint2Byte(ui>>8)]) + m_auiS[3][Uint2Byte(ui)];
}
...
CBlowFish::CBlowFish()
{
	...
	m_nKeyLength = KEY_LENGTH;
}

//Initialize the P and S boxes for a given Key
bool CBlowFish::Initialize(char const* szKey, int nKeyLen, int nMode, int nPadding)
{
	...
	int i, j;
	for(i=0,j=0; i<m_nKeyLength; i++,j=(j+1)%nKeyLen)
		m_szKey[i] = szKey[j];
	m_szKey[m_nKeyLength] = '\0';
	...
	unsigned char* p = reinterpret_cast<unsigned char*>(&m_szKey[0]);
	unsigned int x=0;
	...
	for(i=0; i<18; i++)
	{
		...
		for(int n=4; n--; )
		{
			...
			x |= *(p++);
			iCount++;
			if(iCount == m_nKeyLength)
			{
				//All bytes used, so recycle bytes 
				iCount = 0;
				p = reinterpret_cast<unsigned char*>(&m_szKey[0]);
			}
		}
		m_auiP[i] ^= x;
	}
...
bool CBlowFish::Encrypt(char const* szIn, char* szOut, UINT uiLen)
{
	if(false==m_bInit)
		return false;
	//n should be > 0 and multiple of m_nBlockSize
	if(uiLen<1 || uiLen%m_nBlockSize!=0)
		return false;

	UINT n = uiLen;
	char *pszIn = NULL,*psz=NULL,*pszOut=NULL,*psz1 = NULL;
	pszIn = new char[n];
	psz = pszIn;
	pszOut = new char[n];
	psz1 = pszOut;
	...
	SBlock work;
	if(CBC == m_nMode) //CBC mode, using the Chain
	{
		SBlock chain(m_oChain);
		for(; n >= 8; n -= 8, pszIn += 8)
		{
			BytesToBlock(reinterpret_cast<unsigned char const*>(pszIn), work);
			work ^= chain;
			Encrypt(work);
			chain = work;
			BlockToBytes(work, reinterpret_cast<unsigned char*>(pszOut+=8));
		}
	}
	else if(CFB == m_nMode) //CFB mode, using the Chain
	{
		SBlock chain(m_oChain);
		for(; n >= 8; n -= 8, pszIn += 8)
		{
	...
	Binary2Hex((unsigned char *)psz1,uiLen,szOut);
...
bool CBlowFish::Decrypt(char const* szIn, char* szOut, UINT uiLen)
{
	if(false==m_bInit || strlen(szIn) != 2*uiLen)
		return false;

	UINT n = uiLen;
	//n should be > 0 and multiple of m_nBlockSize
	if(n<1 || n%m_nBlockSize!=0)
		return false;
	
	char *pszIn = NULL,*psz=NULL;
	pszIn = new char[n+1];
	psz = pszIn;
	if(!Hex2Binary(szIn,(unsigned char*)pszIn,n))
	{
		delete psz;
		return false;
	}
	...
	if(CBC == m_nMode) //CBC mode, using the Chain
	{
		...
		for(; n >= 8; n -= 8, pszIn += 8)
		{
			...
			work ^= chain;
			chain = crypt;
			BlockToBytes(work, reinterpret_cast<unsigned char*>(szOut+=8));
		}
	}
	else if(CFB == m_nMode) //CFB mode, using the Chain, not using Decrypt()
	{
		SBlock crypt, chain(m_oChain);
		for(; n >= 8; n -= 8, pszIn += 8)
		{
